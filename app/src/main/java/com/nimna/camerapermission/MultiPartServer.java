...
public class MultiPartServer {
    ...
    public static String postData(URL url, List<NameValuePair> nameValuePairs) throws IOException, ProtocolException {
        ...
        // Write Avatar (if any)
        if(avatarName != null && IMAGE_PATH != null) {
            dataOutputStream.writeBytes(twoHyphens + boundary + crlf);
            dataOutputStream.writeBytes("Content-Disposition: form-data; name=\"" + avatarName + "\";filename=\"" + new File(IMAGE_PATH).getName() + "\";" + crlf);
